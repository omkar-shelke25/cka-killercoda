...
- `targetRef`: Points to the deployment to be managed
- `updatePolicy.updateMode: Recreate`: Recreates pods to apply recommendations
- `containerPolicies`: Defines per-container resource policies
- `controlledResources`: CPU and memory are managed
- `controlledValues: RequestsAndLimits`: Both requests and limits are updated
- `minAllowed` / `maxAllowed`: Bounds for resource recommendations
