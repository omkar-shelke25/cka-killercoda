...
# Task 1: Check if resources.yaml exists
...
# Check if resources.yaml is not empty
...
# Check if resources.yaml contains cert-manager CRDs
...
if ! python3 -c "import yaml; yaml.safe_load_all(open('${RESOURCES_FILE}'))" 2>/dev/null; then
  # Try with yq if available
  if command -v yq &>/dev/null; then
    if ! yq eval '.' "${RESOURCES_FILE}" > /dev/null 2>&1; then
      echo "⚠️  Warning: File may not be valid YAML format"
    else
      echo "✅ File is valid YAML format"
    fi
  else
    echo "✅ File appears to be in YAML format (full validation skipped)"
  fi
else
  echo "✅ File is valid YAML format"
fi

# Task 2: Check if subject.yaml exists
echo ""
...
# Check if subject.yaml is not empty
...
# Check if subject.yaml contains kubectl explain output
...
# Show sample content
echo ""
echo "📋 Sample from resources.yaml (first 10 lines):"
head -10 "${RESOURCES_FILE}"

echo ""
echo "📋 Sample from subject.yaml (first 15 lines):"
...
echo ""
echo "📊 Summary:"
echo "   ✅ resources.yaml contains ${CRD_COUNT} cert-manager CRD(s)"
echo "   ✅ subject.yaml contains Certificate spec.subject documentation"
